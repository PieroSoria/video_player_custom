// Adapted from video_player_media_kit 2.0.0 for Windows.
//
// Copyright © 2023 & onwards, Hitesh Kumar Saini <saini123hitesh@gmail.com>.
// All rights reserved.
// Use of this source code is governed by the MIT license in LICENSE.media_kit.
import 'dart:async';
...
class WindowsVideoPlayer extends VideoPlayerPlatform {
  ...
  Future<void> init() async {
    // The shared controller does not await init. Snapshot before the first
    // await so a newly created player cannot be disposed by initialization.
    final existingPlayers = _players.keys.toList();
    await const MethodChannel('video_player_pip').invokeMethod<void>('reset');
    for (final textureId in existingPlayers) {
  ...
  Future<int?> create(DataSource dataSource) async {
    ...
    // media_kit attaches its native texture after a frame. Request one even
    // when the application is otherwise idle and awaiting initialize().
    ...
    try {
      ...
      // Deliver creation failures through the buffered event stream as well,
      // allowing the controller to receive its ID and dispose the player.
      streamController.addError(
        PlatformException(code: 'video_error', message: error.toString()),
      );
    }
    ...
    if (player != null &&
      ...
      streamSubscriptions.add(
        player.stream.error.listen((event) async {
          if (streamController.isClosed) return;
          streamController.addError(
            PlatformException(code: 'video_error', message: event),
          );
        }),
      );
    }
