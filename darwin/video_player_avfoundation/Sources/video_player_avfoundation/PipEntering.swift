...
extension VideoPlayerPipPlugin {
  ...
  func enterPipMode(playerId: Int, completion: @escaping FlutterResult) {
    ...
    // The vendored video_player_avfoundation plugin publishes the EXACT
    // AVPlayerLayer each inline platform view renders into, keyed by player id.
    // Use it directly; only fall back to the (legacy) view-hierarchy search if
    // the layer isn't registered yet (e.g. texture-based player).
    let playerLayer =
      VideoPlayerPlugin.playerLayer(for: Int64(playerId))
      ?? findAVPlayerLayer(playerId: playerId)
