...
#import "../video_player_custom_avfoundation_objc/include/video_player_custom_avfoundation_objc/VPCDisplayLink.h"

@import Foundation;
@import QuartzCore;

/// A proxy object to act as a CADisplayLink target, to avoid retain loops, since VPCCADisplayLink
/// owns its CADisplayLink, but CADisplayLink retains its target.
API_AVAILABLE(ios(4.0), macos(14.0))
@interface VPCDisplayLinkTarget : NSObject
...
@implementation VPCDisplayLinkTarget
- (instancetype)initWithCallback:(void (^)(void))callback {
  self = [super init];
  if (self) {
    _callback = callback;
...
@interface VPCCADisplayLink ()
// The underlying display link implementation.
@property(nonatomic) CADisplayLink *displayLink;
@property(nonatomic) VPCDisplayLinkTarget *target;
@end

@implementation VPCCADisplayLink

- (instancetype)initWithViewProvider:(NSObject<VPCViewProvider> *)viewProvider
                            callback:(void (^)(void))callback {
  self = [super init];
  if (self) {
    _target = [[VPCDisplayLinkTarget alloc] initWithCallback:callback];
#if TARGET_OS_IOS
