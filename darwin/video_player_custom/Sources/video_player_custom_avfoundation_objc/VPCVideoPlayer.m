...
#import "./include/video_player_custom_avfoundation_objc/VPCVideoPlayer.h"
#import "./include/video_player_custom_avfoundation_objc/VPCVideoPlayer_Internal.h"

#import <CoreMedia/CoreMedia.h>
#import <GLKit/GLKit.h>

#import "./include/video_player_custom_avfoundation_objc/AVAssetTrackUtils.h"
...
static NSString *const kVPCAssetVariantsKey = @"variants";
...
static void VPCRegisterKeyValueObservers(NSObject *observer,
...
/// dictionary mapping KVO keys to NSValue-wrapped context pointers.
///
/// This should only be called to balance calls to VPCRegisterKeyValueObservers, as it is an
...
static void VPCRemoveKeyValueObservers(NSObject *observer,
                                       NSDictionary<NSString *, NSValue *> *observations,
                                       NSObject *target) {
  for (NSString *key in observations) {
...
static NSDictionary<NSString *, NSValue *> *VPCGetPlayerObservations(void) {
  return @{
...
static NSDictionary<NSString *, NSValue *> *VPCGetPlayerItemObservations(void) {
  return @{
...
- (instancetype)initWithPlayerItem:(NSObject<VPCAVPlayerItem> *)item
  ...
  NSObject<VPCAVAsset> *asset = item.asset;
        ...
        if ([tracks count] > 0) {
          ...
          void (^trackCompletionHandler)(void) = ^{
            ...
            if ([videoTrack statusOfValueForKey:@"preferredTransform"
              ...
              self->_preferredTransform = VPCGetStandardizedTrackTransform(
...
- (void)dealloc {
  if (_listenersRegistered && !_disposed) {
    ...
    VPCRemoveKeyValueObservers(self, VPCGetPlayerItemObservations(), _player.currentItem);
    VPCRemoveKeyValueObservers(self, VPCGetPlayerObservations(), _player);
  }
}
...
- (void)disposeWithError:(FlutterError *_Nullable *_Nonnull)error {
  ...
  if (_listenersRegistered) {
    ...
    VPCRemoveKeyValueObservers(self, VPCGetPlayerItemObservations(), self.player.currentItem);
    VPCRemoveKeyValueObservers(self, VPCGetPlayerObservations(), self.player);
  }
...
- (void)setEventListener:(NSObject<VPCVideoEventListener> *)eventListener {
  ...
  if (eventListener && !_listenersRegistered) {
    ...
    VPCRegisterKeyValueObservers(self, VPCGetPlayerItemObservations(), item);
    VPCRegisterKeyValueObservers(self, VPCGetPlayerObservations(), _player);
...
const int64_t VPC_TIME_UNSET = -9223372036854775807;

NS_INLINE int64_t VPCCMTimeToMillis(CMTime time) {
  // When CMTIME_IS_INDEFINITE return a value that matches VPC_TIME_UNSET from ExoPlayer2 on Android.
  // Fixes https://github.com/flutter/flutter/issues/48670
  if (CMTIME_IS_INDEFINITE(time)) return VPC_TIME_UNSET;
...
- (AVMutableVideoComposition *)videoCompositionWithTransform:(CGAffineTransform)transform
                                                       asset:(NSObject<VPCAVAsset> *)asset
...
- (void)observeValueForKeyPath:(NSString *)path
  ...
  if (context == timeRangeContext) {
    ...
    for (NSValue *rangeValue in [object loadedTimeRanges]) {
      ...
      [values addObject:@[
        @(VPCCMTimeToMillis(range.start)),
        @(VPCCMTimeToMillis(range.duration)),
      ]];
    }
...
#pragma mark - VPCVideoPlayerInstanceApi
...
- (nullable NSNumber *)position:(FlutterError *_Nullable *_Nonnull)error {
  return @(VPCCMTimeToMillis([_player currentTime]));
}
...
- (void)getVideoTracks:(void (^)(VPCNativeVideoTrackData *_Nullable,
                                 FlutterError *_Nullable))completion {
  NSMutableArray<VPCMediaSelectionVideoTrackData *> *mediaSelectionTracks = [NSMutableArray array];
  ...
  if (@available(iOS 15.0, macOS 12.0, *)) {
    [urlAsset
        loadValuesAsynchronouslyForKeys:@[ kVPCAssetVariantsKey ]
                      completionHandler:^{
                        dispatch_async(dispatch_get_main_queue(), ^{
                          NSError *error = nil;
                          AVKeyValueStatus status =
                              [urlAsset statusOfValueForKey:kVPCAssetVariantsKey error:&error];
                          ...
                          if (status == AVKeyValueStatusLoaded) {
                            ...
                            for (AVAssetVariant *variant in variants) {
                              ...
                              VPCMediaSelectionVideoTrackData *trackData =
                          ...
                          VPCNativeVideoTrackData *result =
                              [VPCNativeVideoTrackData makeWithAssetTracks:nil
...
- (VPCMediaSelectionVideoTrackData *)createVideoTrackDataWithIndex:(NSInteger)index
  ...
  return [VPCMediaSelectionVideoTrackData
...
- (nullable NSArray<VPCMediaSelectionAudioTrackData *> *)getAudioTracks:
  ...
  NSMutableArray<VPCMediaSelectionAudioTrackData *> *mediaSelectionTracks =
  ...
  if (audioGroup.options.count > 0) {
    ...
    for (NSInteger i = 0; i < audioGroup.options.count; i++) {
      ...
      VPCMediaSelectionAudioTrackData *trackData =
          [VPCMediaSelectionAudioTrackData makeWithIndex:i
...
- (int64_t)duration {
  ...
  return VPCCMTimeToMillis([[[_player currentItem] asset] duration]);
}
