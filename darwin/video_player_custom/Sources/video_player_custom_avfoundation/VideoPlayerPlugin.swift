...
protocol DisplayLinkFactory {
  func displayLink(
    with viewProvider: VPCViewProvider,
...
final class DefaultDisplayLinkFactory: DisplayLinkFactory {
  func displayLink(
    ...
    #if os(iOS)
      return VPCCADisplayLink(viewProvider: viewProvider, callback: callback)
    #elseif os(macOS)
      if #available(macOS 14.0, *) {
        return VPCCADisplayLink(viewProvider: viewProvider, callback: callback)
      }
      return VPCCoreVideoDisplayLink(viewProvider: viewProvider, callback: callback)
    #endif
  }
}

/// Non-test implementation of VPCAssetProvider, wrapping a Flutter plugin
/// registrar.
final class DefaultAssetProvider: NSObject, VPCAssetProvider {
...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  private let avFactory: VPCAVFactory
  private let viewProvider: VPCViewProvider
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let factory = NativeVideoViewFactory(
      ...
      playerByIdentifierProvider: {
        [weak instance] (playerIdentifier: Int64) -> VPCVideoPlayer? in
  ...
  convenience init(registrar: FlutterPluginRegistrar) {
    ...
    #endif
    self.init(
      avFactory: VPCDefaultAVFactory(),
      displayLinkFactory: DefaultDisplayLinkFactory(),
      binaryMessenger: messenger,
      textureRegistry: textures,
      viewProvider: VPCDefaultViewProvider(registrar: registrar),
      assetProvider: DefaultAssetProvider(registrar: registrar)
    )
  }

  init(
    avFactory: VPCAVFactory,
    displayLinkFactory: DisplayLinkFactory,
    binaryMessenger: FlutterBinaryMessenger,
    textureRegistry: FlutterTextureRegistry,
    viewProvider: VPCViewProvider,
    assetProvider: VPCAssetProvider
  ) {
  ...
  func createPlatformViewPlayer(options params: CreationOptions) throws -> Int64 {
    ...
    let player = VPCVideoPlayer(playerItem: item, avFactory: avFactory, viewProvider: viewProvider)
  ...
  func createTexturePlayer(options creationOptions: CreationOptions) throws -> TexturePlayerIds {
    ...
    let frameUpdater = VPCFrameUpdater(registry: textureRegistry)
    let displayLink = displayLinkFactory.displayLink(with: viewProvider) {
      frameUpdater.displayLinkFired()
    }

    let player = VPCTextureBasedVideoPlayer(
      playerItem: item,
  ...
  private func configurePlayer(
    ...
    SetUpVPCVideoPlayerInstanceApiWithSuffix(binaryMessenger, player, channelSuffix)
    ...
    let eventBridge = VPCEventBridge(
      messenger: binaryMessenger,
      channelName: "flutter.dev/videoPlayerCustom/videoEvents\(channelSuffix)"
    )
  ...
  private func playerItem(with options: CreationOptions) throws -> VPCAVPlayerItem {
