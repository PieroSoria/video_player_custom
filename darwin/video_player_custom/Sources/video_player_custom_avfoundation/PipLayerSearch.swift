...
extension VideoPlayerPipPlugin {
  ...
  func findAVPlayerLayerInView(_ view: UIView, depth: Int) -> AVPlayerLayer? {
    ...
    // Check for class name matching VPCPlayerView which has an AVPlayerLayer as its layer
    if className.contains("VPCPlayerView") {
      pipLog("\(indentation)VideoPlayerPip: Found VPCPlayerView")
      if let playerLayer = view.layer as? AVPlayerLayer {
        pipLog("\(indentation)VideoPlayerPip: VPCPlayerView's layer is AVPlayerLayer")
        if let player = playerLayer.player {
          pipLog("\(indentation)VideoPlayerPip: VPCPlayerView's AVPlayerLayer has player: \(player)")
        } else {
          pipLog("\(indentation)VideoPlayerPip: VPCPlayerView's AVPlayerLayer has no player set")
        }
        return playerLayer
      } else {
        pipLog("\(indentation)VideoPlayerPip: VPCPlayerView's layer is not AVPlayerLayer: \(type(of: view.layer))")
      }
    }
